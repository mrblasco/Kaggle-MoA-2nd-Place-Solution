...
def run_training(fold, seed):
    ...
    tabnet_params = dict(n_d = 64
        ...
        , optimizer_params = dict(lr = params.learning_rate, weight_decay = params.weight_decay)  # 2e-2, 1e-5
        , mask_type = "entmax"
        , scheduler_params = dict(mode = "min"
                                  , patience = 5
                                  , min_lr = params.learning_rate #1e-5
                                  , factor = 0.9)
...
def run_k_fold(num_folds, seed):
    oof = np.zeros((len(train), len(target_cols)))
    predictions = np.zeros((len(test), len(target_cols)))

    for fold in range(num_folds):
        logging.info("Fold {} out of {}".format(fold+1, params.num_folds))
        oof_, pred_ = run_training(fold, seed)

        predictions += pred_ / params.num_folds
        oof += oof_
...
if __name__ == '__main__':
  ...
  # List with target names 
  target_cols = target.drop('sig_id', axis=1).columns.values.tolist()

  # Target signatures merge with drug ID
  ...
  for seed in range(params.num_seeds):
      ...
      target2 = target.copy()
      dct1 = {}; dct2 = {}
      skf = MultilabelStratifiedKFold(n_splits = params.num_folds) # , shuffle = True, random_state = seed
